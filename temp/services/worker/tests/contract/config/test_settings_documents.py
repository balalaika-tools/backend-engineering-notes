"""Configuration-document contracts for the worker."""
...
from worker.config.settings import YAML_POLICY_FIELDS, Settings
...
PASSTHROUGH_OPTIONAL = {"WORKER_CONFIG_DIR"}
...
def _merged_policy(environment: str) -> dict[str, object]:
    paths = [
        ...
        ROOT / "config" / "services" / "worker.yaml",
        ROOT / "config" / "services" / f"worker.{environment}.yaml",
    ]
...
def test_dev_yaml_baselines_exist() -> None:
    ...
    assert (ROOT / "config" / "services" / "worker.dev.yaml").is_file()
...
def test_root_env_documents_all_required_worker_inputs() -> None:
    root_variables = {
        match.group(1)
